...
int main(int argc, char **argv) {
    ...
    for (unsigned int i = 0; i < devices.size(); i++) {
        ...
        if (err != CL_SUCCESS) {
            ...
            OCL_CHECK(err,
                      user_kernel = cl::Kernel(program, "accelerator_SIFT_M16_batch", &err));
            valid_device++;
	...
	std::string data_dir_prefix = "/mnt/scratch/wenqi/Faiss_Enzian_U250_index/SIFT1000M_IVF32768,PQ16";
    ...
    std::string product_quantizer_dir_suffix("product_quantizer_float32_16_256_8_raw");
