...
// Usage (e.g.): ./FPGA_simulator 10.253.74.24 8881 5001 1 32
...
void thread_recv_packets(
    ...
    int recv_bytes_per_query = 64 * (size_header + size_cell_IDs + size_query_vector + nprobe * size_center_vector);
    std::cout << "recv_bytes_per_query: " << recv_bytes_per_query << std::endl;
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        while (total_recv_bytes < recv_bytes_per_query) {
            ...
            int recv_bytes = read(sock, FPGA_input.data() + query_id * recv_bytes_per_query + total_recv_bytes, recv_bytes_this_iter);
