...
// Usage (e.g.): ./CPU_to_single_FPGA 10.253.74.24 8881 5001 128 100 32 100 16 10 0
//  "Usage: " << argv[0] << " <1 FPGA_IP_addr> <2 C2F_port> <3 F2C_port> <4 D> <5 TOPK> <6 batch_size> <7 total_batch_num> <8 nprobe> <9 query_window_size> <10 enable_inter_batch_window>" << std::endl;
...
{ 
  //////////     Parameter Init     //////////  
  std::cout << "Usage: " << argv[0] << " <1 FPGA_IP_addr> <2 C2F_port> <3 F2C_port> <4 D> <5 TOPK> <6 batch_size> <7 total_batch_num> <8 nprobe> <9 window_size> <10 enable_inter_batch_window>" << std::endl;
  ...
  int enable_inter_batch_window = 1;
  if (argc >= 11)
  {
    enable_inter_batch_window = strtol(argv[10], NULL, 10);
  } 
