...
// Usage (e.g.): ./host_single_thread_same_conn 10.253.74.24 5001 1 32
...
#define SEND_RECV_PKG_SIZE 4096 // 1024 
...
void thread_send_recv_packets(
    // value inputs
    const char* IP_addr, unsigned int send_recv_port, int query_num, int nprobe, 
    int recv_bytes_per_query, int send_recv_query_gap, 
    // array inputs
...
{ 
    ...
    unsigned int send_recv_port = 5001;
    if (argc >= 3)
    {
        send_recv_port = strtol(argv[2], NULL, 10);
    } 
    ...
    std::thread t_send_recv(
        thread_send_recv_packets, 
        // value inputs
        IP_addr, send_recv_port, query_num, nprobe,
        recv_bytes_per_query, send_recv_query_gap,
        // array inputs
        query_vectors.data(), vector_quantizer.data(), alg_hnswlib,
        // output
        out_buf,
        // profiling pointers
        query_start_time.data(), query_finish_time.data());
...
void thread_send_recv_packets(
    ...
    size_t FPGA_input_bytes = query_num * send_bytes_per_query;
    std::vector<char ,aligned_allocator<char >> FPGA_input(FPGA_input_bytes);
 
    printf("Printing send_recv_port from Thread %d\n", send_recv_port); 
    ...
    serv_addr.sin_port = htons(send_recv_port); 
    ...
    for (int query_id = 0; query_id < query_num; query_id++) {
        ...
        *(query_start_time_ptr + query_id) = std::chrono::system_clock::now();
        ...
        while (total_sent_bytes < send_bytes_per_query) {
            int send_bytes_this_iter = (send_bytes_per_query - total_sent_bytes) < SEND_RECV_PKG_SIZE? (send_bytes_per_query - total_sent_bytes) : SEND_RECV_PKG_SIZE;
        ...
        while (total_recv_bytes < recv_bytes_per_query) {
            int recv_bytes_this_iter = (recv_bytes_per_query - total_recv_bytes) < SEND_RECV_PKG_SIZE? (recv_bytes_per_query - total_recv_bytes) : SEND_RECV_PKG_SIZE;
...
//             }
    ...
    std::cout << "Send/recv thread Duration (us) = " << durationUs << std::endl;
    std::cout << "Send/recv thread QPS = " << query_num / (durationUs / 1000.0 / 1000.0) << std::endl;

    std::cout << "Send/recv thread finished." << std::endl;
    return; 
} 
